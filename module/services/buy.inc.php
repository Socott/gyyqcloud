...
if($submit) {
			...
			if($t && $t['status'] == 3 && $t['username'] != $_username && $t['amount'] > 0) {
				...
				if($express && in_array($express, array(1,2,3))) {
					...
					if($express_id && $area_id) {
						$E = $db->get_one("SELECT * FROM {$DT_PRE}services_express WHERE itemid=$express_id");
						if($E && $E['items'] > 0) {
							$AREA = cache_read('area.php');
							$aid = $area_id;
							$ii = 0;
							do {
								$E = $db->get_one("SELECT * FROM {$DT_PRE}services_express WHERE parentid=$express_id AND areaid=$aid");
								if($E) {
				...
				$db->query("INSERT INTO {$DT_PRE}services_order (mid,mallid,buyer,seller,title,thumb,price,number,amount,addtime,updatetime,note, buyer_postcode,buyer_address,buyer_name,buyer_phone,buyer_mobile,status,fee_name,fee,cod) VALUES ('$moduleid','$itemid','$_username','$t[username]','$title','$t[thumb]','$price','$number','$amount','$DT_TIME','$DT_TIME','$note','$buyer_postcode','$buyer_address','$buyer_name','$buyer_phone','$buyer_mobile','$status','$fee_name','$fee','$cod')");
				$oid = $db->insert_id();
				$db->query("REPLACE INTO {$DT_PRE}services_comment (itemid,mallid,buyer,seller) VALUES ('$oid','$itemid','$_username','$t[username]')");
				$tmp = $db->get_one("SELECT mallid FROM {$DT_PRE}services_stat WHERE mallid=$itemid");
				if(!$tmp) $db->query("REPLACE INTO {$DT_PRE}services_stat (mallid,buyer,seller) VALUES ('$itemid','$_username','$t[username]')");
				if(!$cod) $ids .= ','.$oid;
				//send message
				$touser = $t['username'];
				$_title = $title;
				$title = lang($L['trade_message_t6'], array($oid));
				$url = $MODULE[2]['linkurl'].'trade_services.php?itemid='.$oid;
	...
	if($action == 'show') {
		...
		$forward = $MODULE[2]['linkurl'].'trade_services.php?'.($forward ? $forward : 'action=order');
	} else {
