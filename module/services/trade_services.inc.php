	...
	switch($step) {
		...
		case 'pay'://买家付款
			...
			if($submit) {
				...
				//更新商品数据
				if($td['mid'] == 16) {
					$db->query("UPDATE {$DT_PRE}services SET orders=orders+1,sales=sales+$td[number],amount=amount-$td[number] WHERE itemid=$mallid");
				} else {
		...
		case 'comment'://交易评价
			...
			if($_username == $td['seller']) {
				...
				if($submit) {
					...
					$s = 'b'.$star;
					$db->query("UPDATE {$DT_PRE}services_comment SET buyer_star=$star,buyer_comment='$content',buyer_ctime=$DT_TIME WHERE itemid=$itemid");
					$db->query("UPDATE {$DT_PRE}services_stat SET bcomment=bcomment+1,`$s`=`$s`+1 WHERE mallid=$mallid");
					message($L['trade_msg_comment_success'], $forward);
				}
			} else if($_username == $td['buyer']) {
				if($td['seller_star']) message($L['trade_msg_comment_again']);
				if($submit) {
					$db->query("UPDATE {$DT_PRE}services SET comments=comments+1 WHERE itemid=$mallid");
					$db->query("UPDATE {$table} SET seller_star=$star WHERE itemid=$itemid");
					$s = 's'.$star;
					$db->query("UPDATE {$DT_PRE}services_comment SET seller_star=$star,seller_comment='$content',seller_ctime=$DT_TIME WHERE itemid=$itemid");
					$db->query("UPDATE {$DT_PRE}services_stat SET scomment=scomment+1,`$s`=`$s`+1 WHERE mallid=$mallid");
					message($L['trade_msg_comment_success'], $forward);
				}
			}
		break;
		case 'comment_detail'://评价详情
			if($td['mid'] != 16) message($L['trade_msg_deny_comment']);
			$cm = $db->get_one("SELECT * FROM {$DT_PRE}services_comment WHERE itemid=$itemid");
			if($submit) {
				$content = dhtmlspecialchars($content);
				$content or message($L['trade_msg_empty_explain']);
				if($_username == $td['seller']) {
					if($cm['buyer_reply']) message($L['trade_msg_explain_again']);
					$db->query("UPDATE {$DT_PRE}services_comment SET buyer_reply='$content',buyer_rtime=$DT_TIME WHERE itemid=$itemid");
				} else {
					if($cm['seller_reply']) message($L['trade_msg_explain_again']);
					$db->query("UPDATE {$DT_PRE}services_comment SET seller_reply='$content',seller_rtime=$DT_TIME WHERE itemid=$itemid");
				}
