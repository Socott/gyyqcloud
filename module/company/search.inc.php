...
$condition=" 1=1 ";
	if($kw) {
		...
		if(strlen($kw) < 1 || strlen($kw) > $DT['max_kw']) 
	...
	$fds = $MOD['fields'];
	$condition .= " and groupid>5 AND catids<>''";
	...
	if($items) 
	{
		...
		$result = $db->query("SELECT $fds,homepage,sell FROM {$table} WHERE {$condition}{$order} LIMIT {$offset},{$pagesize}", $DT['cache_search'] && $page == 1 ? 'CACHE' : '', $DT['cache_search']);
		if($kw) {
			$replacef = explode(' ', $kw);
...
foreach($tags as $k=>$v)
{
	...
	$result = $db->query("SELECT itemid,title,price,thumb,linkurl FROM  lsg_mall WHERE thumb<>'' and username<>'".$v['username']."'  order by RAND() desc limit 3");
		while($r = $db->fetch_array($result)) {
			$tags[$k]['product'][]= $r;
		}
	$db->free_result($result);
