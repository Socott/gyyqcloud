...
if($submit) {
			...
			if($t && $t['status'] == 3 && $t['username'] != $_username && $t['amount'] > 0) {
				...
				if($express && in_array($express, array(1,2,3))) {
					...
					if($express_id && $area_id) {
						$E = $db->get_one("SELECT * FROM {$DT_PRE}softpool_express WHERE itemid=$express_id");
						if($E && $E['items'] > 0) {
							$AREA = cache_read('area.php');
							$aid = $area_id;
							$ii = 0;
							do {
								$E = $db->get_one("SELECT * FROM {$DT_PRE}softpool_express WHERE parentid=$express_id AND areaid=$aid");
								if($E) {
	...
	if($action == 'show') {
		...
		$forward = $MODULE[2]['linkurl'].'trade_softpool.php?'.($forward ? $forward : 'action=order');
	} else {
