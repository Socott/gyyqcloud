...
if($submit) {
	...
	if($post) {
		...
		$demand = $add['demand'];
		$service_type = $add['service_type'];
		$current_descript = $add['current_descript'];
			...
			if($t && $t['status'] == 3 && $t['username'] != $_username && $t['amount'] > 0) {
				...
				if($express && in_array($express, array(1,2,3))) {
					...
					if($express_id && $area_id) {
						$E = $db->get_one("SELECT * FROM {$DT_PRE}expertpool_express WHERE itemid=$express_id");
						if($E && $E['items'] > 0) {
							$AREA = cache_read('area.php');
							$aid = $area_id;
							$ii = 0;
							do {
								$E = $db->get_one("SELECT * FROM {$DT_PRE}expertpool_express WHERE parentid=$express_id AND areaid=$aid");
								if($E) {
				...
				$db->query("INSERT INTO {$DT_PRE}expertpool_order (mid,mallid,buyer,seller,title,thumb,price,number,amount,addtime,updatetime,note, buyer_postcode,buyer_address,buyer_name,buyer_phone,buyer_mobile,status,fee_name,fee,cod,demand,service_type,current_descript) VALUES ('$moduleid','$itemid','$_username','$t[username]','$title','$t[thumb]','$price','$number','$amount','$DT_TIME','$DT_TIME','$note','$buyer_postcode','$buyer_address','$buyer_name','$buyer_phone','$buyer_mobile','$status','$fee_name','$fee','$cod','$demand','$service_type','$current_descript')");
				$oid = $db->insert_id();
				$db->query("REPLACE INTO {$DT_PRE}expertpool_comment (itemid,mallid,buyer,seller) VALUES ('$oid','$itemid','$_username','$t[username]')");
				$tmp = $db->get_one("SELECT mallid FROM {$DT_PRE}expertpool_stat WHERE mallid=$itemid");
				if(!$tmp) $db->query("REPLACE INTO {$DT_PRE}expertpool_stat (mallid,buyer,seller) VALUES ('$itemid','$_username','$t[username]')");
				if(!$cod) $ids .= ','.$oid;
				//send message
				$touser = $t['username'];
				$_title = $title;
