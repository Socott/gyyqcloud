<?php
/*
	[Destoon B2B System] Copyright (c) 2008-2016 www.gyycloud.com
...
if($userid || $username) {
	...
	switch($action) {
		...
		case 'overroll':
			...
			$head_title = $head_name.$DT['seo_delimiter'].$head_title;
			$back_link = $HURL;
			$overroll = $db->get_one("SELECT item_value as overroll FROM {$DT_PRE}company_setting WHERE item_key='overroll' and userid =$userid ");
			if($overroll)$HOME['overroll'] =$overroll['overroll'] ;
			include template('homepage-'.$action, 'mobile');
		break;
		case 'link':
			isset($_MENU[$action]) or dheader($HURL);
